...
func (view *ShardView) cloneShardViewFrom(target chain.ChainViewInterface) error {
	tempMarshal, err := json.Marshal(target)
	if err != nil {
		return common.NewBlockChainError(common.MashallJsonShardBestStateError, fmt.Errorf("Shard Best State %+v get %+v", target.GetHeight(), err))
	}
	err = json.Unmarshal(tempMarshal, view)
	if err != nil {
		return common.NewBlockChainError(common.UnmashallJsonShardBestStateError, fmt.Errorf("Clone Shard Best State %+v get %+v", target.GetHeight(), err))
	}
	if reflect.DeepEqual(*view, ShardView{}) {
		return common.NewBlockChainError(common.CloneShardBestStateError, fmt.Errorf("Shard Best State %+v clone failed", target.GetHeight()))
	}
	return nil
}
...
func (view *ShardView) SetCommittee(newCommittee []incognitokey.CommitteePublicKey) error {
	if len(newCommittee) > view.MaxCommitteeSize {
		return common.NewBlockChainError(common.ShardCommitteeLengthAndCommitteeIndexError, fmt.Errorf("newCommittee lenght: %v MaxCommitteeSize: %v", len(newCommittee), view.MaxCommitteeSize))
	}
	if len(newCommittee) < view.MinCommitteeSize {
		return common.NewBlockChainError(common.ShardCommitteeLengthAndCommitteeIndexError, fmt.Errorf("newCommittee lenght: %v MaxCommitteeSize: %v", len(newCommittee), view.MinCommitteeSize))
	}
	res := []byte{}
...
func (view ShardView) CloneViewFrom(new_view *chain.ChainViewInterface) error {
	return nil
}
...
func (view ShardView) ConnectBlockAndCreateView(block common.BlockInterface) (chain.ChainViewInterface, error) {
	return nil, nil
}
